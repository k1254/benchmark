...
plot "fio_seq.txt" using ($0*3+0):2         with boxes lw 0.5 lc rgb "light-cyan"  title "Linux Machine",\
"fio_seq.txt" using ($0*3+1):3:xtic(1) with boxes lw 0.5 lc rgb "light-green" title "KVM",\
"fio_seq.txt" using ($0*3+2):4         with boxes lw 0.5 lc rgb "light-pink"  title "Docker"